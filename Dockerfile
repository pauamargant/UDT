...
ENV PROJECT_ROOT="/workspace/iccluster040_scratch/students/pauamargant/RoomAgents"
ENV BUILD_SRC_ROOT="/opt/roomagents-src"
...
RUN mkdir -p "${PROJECT_ROOT}" "${BUILD_SRC_ROOT}"
...
# 6. Copy project source from build context into a build-only directory.
# PROJECT_ROOT is left untouched in case it already exists.
COPY . ${BUILD_SRC_ROOT}
...
RUN cd ${BUILD_SRC_ROOT}/splat_solver && \
    python -m pip install -e . --no-build-isolation
RUN cd ${BUILD_SRC_ROOT}/splat_solver/submodules/segment-anything-langsplat && \
    MAX_JOBS=4 python -m pip install . --no-build-isolation
RUN cd ${BUILD_SRC_ROOT}/splat_solver/submodules/gsplat && \
    MAX_JOBS=4 python -m pip install . --no-build-isolation
RUN cd ${BUILD_SRC_ROOT}/splat_solver/submodules/bsplat && \
    MAX_JOBS=4 python -m pip install . --no-build-isolation
RUN cd ${BUILD_SRC_ROOT}/splat_solver/submodules/gsplat_ext && \
    MAX_JOBS=4 python -m pip install . --no-build-isolation
...
RUN conda run -n splat-distiller-feat env LD_LIBRARY_PATH=/opt/conda/envs/splat-distiller-feat/lib MAX_JOBS=4 \
    python -m pip install ${BUILD_SRC_ROOT}/splat_solver/submodules/gsplat --no-build-isolation
RUN conda run -n splat-distiller-feat env LD_LIBRARY_PATH=/opt/conda/envs/splat-distiller-feat/lib MAX_JOBS=4 \
    python -m pip install ${BUILD_SRC_ROOT}/splat_solver/submodules/bsplat --no-build-isolation
RUN conda run -n splat-distiller-feat env LD_LIBRARY_PATH=/opt/conda/envs/splat-distiller-feat/lib MAX_JOBS=4 \
    python -m pip install ${BUILD_SRC_ROOT}/splat_solver/submodules/gsplat_ext --no-build-isolation
...
RUN groupadd --gid ${USER_GID} ${USERNAME} && \
    ...
    chown -R ${USERNAME}:${USERNAME} ${PROJECT_ROOT} ${BUILD_SRC_ROOT} /opt/conda
